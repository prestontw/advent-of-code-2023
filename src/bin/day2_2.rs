...
fn run(input: &str) -> usize {
    ...
    list.map(|game| game.min_set_cubes().power()).sum()
}
...
impl Game {
    ...
    fn min_set_cubes(&self) -> Roll {
        self.shown.iter().fold(
            ...
            |min_set, roll| Roll {
                green: min_set.green.max(roll.green),
                red: min_set.red.max(roll.red),
                blue: min_set.blue.max(roll.blue),
            },
        )
    }
}
