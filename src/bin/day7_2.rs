...
impl Hand {
    fn new(hand: Vec<Value>) -> Self {
        ...
        let (jokers, others): (Vec<_>, Vec<_>) =
            hand.clone().into_iter().partition(|v| v == &Value::Jack);
        let mut value_counts = counts(&others).values().copied().collect::<Vec<_>>();
        let max = value_counts.iter_mut().max().unwrap();
        *max += jokers.len();
        let counts_counts = counts(value_counts);
